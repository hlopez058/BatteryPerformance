function [ output ] = nn_training( A,NNType)
...
t_z = zeros(length(dv1),1)';
t_o = ones(length(dv1),1)';
...
% Using resilient backpropagation and mean square error
if(NNType==20)
    net = patternnet(hiddenLayerSize,trainrp);
end
if(NNType==21)
    net = patternnet(hiddenLayerSize,trainrp,mse);
end
% Radial basis with resilient backpropagation
if(NNType==22)
    net = patternnet(hiddenLayerSize,trainrp);
